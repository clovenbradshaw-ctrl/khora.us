...
- `tools/README.md` adds `hydration.ts` (`khora-hyd`) — CLI wrapper for `exportHydration` / `importHydration` with `export`, `import`, `inspect` (no key needed), and `verify` (no decryption) subcommands; passphrase / hardware-key / split-key modes. Status note updated to land `hydration.ts` alongside `snapshot.ts` in Phase 4.
